...
<div class="panel-heading">ARE YOU SURE YOU WANT TO DELETE VOUCHER WITH ID: <?PHP print $del_id;?></div>
  <div class="panel-body">
    <a class="btn btn-info" href="show_entry.php">CANCEL</a>&nbsp;<a class="btn btn-success" href="show_entry.php?delid=<?php echo $del_id; ?>&action=<?php echo 'confirm'; ?>">CONFIRM</a>
  </div>
</div>
		  ...
		  if($action=='confirm')
		  {
			  if (!empty($_GET['delid'])) {
						   $del_id=$_GET['delid'];
						   $backup_query="insert into backup_entryitems(entry_id,ledger_id,amount,dc,created_by) select entry_id,ledger_id,amount,dc,created_by from entryitems where entryitems.entry_id=$del_id" ;
						   $db->query($backup_query);
						   $up_query="update backup_entryitems set deleted_by='$suid'  where entry_id=$del_id";
						   $db->query($up_query);
						   $del_query="delete from entryitems where entry_id=$del_id"; 
			               $db->query($del_query);
						   ...
						   $db->query($up_query);
						   			   $del_query="delete from entries where id=$del_id"; 
			               $db->query($del_query);
		  }
...
if($hidvouchertype==0)
{
$select = $db->query("SELECT e.*,t.name as tname FROM entries e, entrytypes t where (e.created_by='$suid' or e.created_by in(select  userid from usertable where created_by='$suid')) and e.entrytype_id=t.id ORDER BY e.id DESC");
}
else
{
$select = $db->query("SELECT e.*,t.name as tname FROM entries e, entrytypes t where (e.created_by='$suid' or e.created_by in(select  userid from usertable where created_by='$suid')) and  e.entrytype_id=t.id and e.entrytype_id=$hidvouchertype ORDER BY e.id DESC");
}
...
*/
	...
	<input type="hidden" name="hidvouchertype" >
            ...
            <tr>
              <td>
			<?PHP $quer_type=$db->query("SELECT id ,name from entrytypes order by id asc"); ?>
...
if($hidvouchertype==0)
{
				...
				$query = $db->query( "SELECT e.*,t.name as tname FROM entries e, entrytypes t where (e.created_by='$suid' or e.created_by in(select  userid from usertable where created_by='$suid')) and   e.entrytype_id=t.id   ORDER BY e.id DESC LIMIT $start, $num_of_records_per_page");
				}
else
{
				$query = $db->query("SELECT e.*,t.name as tname FROM entries e, entrytypes t where (e.created_by='$suid' or e.created_by in(select  userid from usertable where created_by='$suid')) and   e.entrytype_id=t.id  and e.entrytype_id=$hidvouchertype ORDER BY e.id DESC LIMIT $start, $num_of_records_per_page");
...
//$result02=mysql_query($con,$query02);	//print '*'.$vouchertype.'*';		?>
			...
			<table class="table table-bordered" id="chartofaccount">
				...
				<?php
					...
					$id=$row->id;
					$sql2="select l.name,m.dc FROM entries e, entryitems m, ledgers l where e.id=$id and e.id=m.entry_id and m.ledger_id=l.id";
					//print $sql2;
