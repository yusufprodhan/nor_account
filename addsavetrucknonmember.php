...
$valc="";
 ...
 $txtnumber 		= $_POST['txtnumber'];
				...
				{
					$cdt=date('Y-m-d');
					$sql="insert into truck_info(id,truck_no,mem_id,truck_type,include_date,mem_type)values('','$truck_no','Nonmember','A','$cdt',2)";
		...
		$sql="select * from truck_info where trim(truck_no)='$truck_no'";
				...
				$sql="select id from ledgers where ((name like '%NonMem%'))";
					$result=mysqli_query($con,$sql);
					$row=mysqli_fetch_row($result);
					$nonmem=$row[0];
					$sql="insert into entryitems(entry_id,entrytype_id,ledger_id,amount,dc,itemnarat,truck_id,created_by)values($txtnumber,$hid,$nonmem,$amt,'C','$truck_no',$truck_id,'$previllageid')";
					$result=mysqli_query($con,$sql);
	...
	//$result=mysqli_query($con,$sql);
					  ...
					  header("location:show_entry.php");
						  ...
						  header("location:addentrypaymentoftrucknonmember.php?hid=$hid      ");
					  }
			 // print $sql;
