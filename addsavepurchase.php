...
$valc="";
 ...
 $txtnumber 		= $_POST['txtnumber'];
		...
		$sql="select max(id) from ledgers";
				...
				$sql="insert into entryitems(entry_id,ledger_id,amount,dc,itemnarat,created_by)values($txtnumber,$item_chart_id,$item_buyprice*$item_quantity,'D','Sales Item','$previllageid')";
					 }
					 
					 
					if($valc=='Add Supplier'){
		...
		$sql="INSERT INTO suppliers (ledger_id,supplier_name,supplier_address,supplier_mobile,supplier_email) VALUES ( $supplier_chart_id,'$sup_name','$sup_addr','$sup_mobile','$sup_email')";
		$result=mysqli_query($con,$sql);
				}
					}
					
					if (isset($_POST['complete'])){
		      if($_POST['complete']=='Complete Purchase Entry'){
				  $txtnumber 		= $_POST['txtnumber'];
				   $txtdate 		= $_POST['txtdate'];
				  ...
				  $bank 		= $_POST['bank'];
					...
					if($cashpay>0){
	$sql="insert into entryitems(entry_id,ledger_id,amount,dc,itemnarat,created_by)values($txtnumber,5,$cashpay,'C','Sales Item','$previllageid')";
	$result=mysqli_query($con,$sql);
	}
	if($chequepay>0){
  ...
  $dbid=$row22['id'];
	...
	$sql="insert into entryitems(entry_id,ledger_id,amount,dc,itemnarat,created_by)values($txtnumber,$dbid,$chequepay,'C','Sales Item','$previllageid')";
	
	$result=mysqli_query($con,$sql);
	}
	$rest=$hdrtotal-$cashpay-$chequepay;
	if($rest>0){
	$sql="insert into entryitems(entry_id,ledger_id,amount,dc,itemnarat,created_by)values($txtnumber,$supplier_chart_id,$rest,'C','Sales Item','$previllageid')";
	...
	//$result=mysqli_query($con,$sql);
					  ...
					  header("location:purchase_entry.php");
						  ...
						  header("location:purchase_entry.php?hid=$hid      ");
					  }
			 // print $sql;
