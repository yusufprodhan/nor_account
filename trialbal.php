	 ...
	 while($row11 = $result11->fetch_object())
      {
		...
		$sql="select * from groups where parent_id=$id_child";
		$result11=mysqli_query($con,$sql);
		 while($row11 = $result11->fetch_object())
      {
	    $id_child_ch=$row11->id;
		$sql2="select sum(op_balance) as bal from ledgers where group_id=$id_child_ch and op_balance_dc='D' group by group_id";
  if( $result22=mysqli_query($con,$sql2)){
   $row22 = mysqli_fetch_row($result22);
   $opnbaldd=$row22[0];}else{$opnbaldd=0;}
      $sql3="select sum(op_balance) as bal from ledgers where group_id=$id_child_ch and op_balance_dc='C' group by group_id";
...
if($pr_id==1){
$sql="select id,name,op_balance,op_balance_dc from ledgers where      id=5 order by id";
$result_led=mysqli_query($con,$sql);
while($row_led = mysqli_fetch_row($result_led))
{
  $lid=$row_led[0];$lname=$row_led[1];
