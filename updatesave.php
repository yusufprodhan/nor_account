...
$valc="";
 ...
 $txtnumber 		= $_POST['txtnumber'];
				  ...
				  //$bank 		= $_POST['bank'];
					...
					$sql="insert into entryitems(entry_id,entrytype_id,ledger_id,amount,dc,created_by)values($txtnumber,$hid,$debit_chart_id,$debit_amount,'D','$previllageid')";
					
					 }
					if($valc=='Add Credit'){
						$credit_chart_id 	= $_POST['credit_chart_id'];
					$credit_amount 		= $_POST['credit_amount'];
					$sql="insert into entryitems(entry_id,entrytype_id,ledger_id,amount,dc,created_by)values($txtnumber,$hid,$credit_chart_id,$credit_amount,'C','$previllageid')";
	...
	//$result=mysqli_query($con,$sql);
		      ...
		      if($_POST['complete_update']=='Complete Journal Entry'){
				  ...
				  $bank 		= $_POST['bank'];
					...
					$hcrtotal 		= $_POST['hcrtotal'];
					$txtnarration 	= $_POST['txtnarration'];
								  $sql="update entries set date='$txtdate',dr_total=$hdrtotal,cr_total=$hcrtotal,narration='$txtnarration',paymentmode=$pmode,bank='$bank',cheque='$cheque',chqdate='$txtchqdate',mobile='$mobile',updated_by='$previllageid' where id=$txtnumber and entrytype_id=$hid";
	print $sql;
	//print"<br>";
	//$result=mysqli_query($con,$sql);
								  }
					  ...
					  header("location:show_entry.php");
						  ...
						  header("location:updateentry.php?hid=$hid&id=$txtnumber");
					  }
			 // print $sql;
