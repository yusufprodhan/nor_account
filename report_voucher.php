											...
											<tbody>
												...
												//foreach ($details as $list) {
													//if ($list['debit']) {
														...
														$sql=$db->query("select m.id,l.name,m.amount,m.itemnarat,m.truck_id from ledgers l, entryitems m where m.ledger_id=l.id and m.entry_id=$entr_id and m.dc='D' and m.refund=0 and m.entrytype_id=$tp");
														if($sql<>NULL){
														while ($row = $sql->fetch_object()) {
														?>
														<tr>
															<td ><?php  if($row->truck_id==0){print $row->name;}else {print '';} ?></td>
...
<?PHP } else if($type=='Receipt')
	{ ?>
									...
									<div id="credit_details">
										<table width="519" class=" table-striped responsive">
											<thead>
												<tr>
													<th width="218" class="center">Receipt Head </th>
													<th width="120" class="center">Amount</th>
                                                    <th width="165"  class="center">Description</th>
											...
											//	foreach ($details as $list) {
												//	if ($list['credit']) {
													...
													$sql2=$db->query("select m.id,l.name,m.amount,m.itemnarat,m.truck_id from ledgers l, entryitems m where m.ledger_id=l.id and m.entry_id=$entr_id and m.dc='C'  and m.refund=0 and m.entrytype_id=$tp");
						if($sql2<>NULL){							
														while ($row2 = $sql2->fetch_object()) {
														?>
														<tr>
															<td><?php if($row2->truck_id==0){print $row2->name;}else {print '';}//echo $list['chart_name']; ?></td>
	...
	<td><?php print $row2->itemnarat;//echo $list['chart_name']; ?></td>														
        ...
        <!-- Table row -->
        <div id="debit_details">
									  <table width="108%" class="table  table-striped responsive">
											<thead>
												<tr>
													<th class="center"> Payment Head </th>
													<th class="center">Amount</th>
											...
											<tbody>
												...
												//foreach ($details as $list) {
													//if ($list['debit']) {
														...
														$sql=$db->query("select m.id,l.name,m.amount,m.itemnarat from ledgers l, entryitems m where m.ledger_id=l.id and m.entry_id=$entr_id and m.dc='D' and m.refund=0 and m.entrytype_id=$tp");
														if($sql<>NULL){
														while ($row = $sql->fetch_object()) {
														?>
														<tr>
